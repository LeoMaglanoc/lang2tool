...
def quat_rotate(q, v):
    ...
    c = q_vec * (q_vec.reshape(shape[0], 1, 3) @ v.reshape(shape[0], 3, 1))[..., 0] * 2.0
    return a + b + c
...
def compute_fk_dict(
    ...
    for link_name in link_names:
        ...
        assert fk_dict[link_name].shape == (
            N,
            4,
            4,
...
def compute_observation(
    ...
    assert prev_action_targets.shape == (
        N,
        J,
    ...
    assert q_lower_limits.shape == (
        J,
    ), f"q_lower_limits.shape: {q_lower_limits.shape}, expected: (J,)"
    assert q_upper_limits.shape == (
        J,
    ...
    N_FINGERTIPS = 5
    assert (
        JOINT_NAMES_ISAACGYM == urdf.actuated_joint_names
    ...
    t6 = time.time()
    assert palm_center_pos.shape == (
        N,
        3,
    ), f"palm_center_pos.shape: {palm_center_pos.shape}, expected: (N, 3)"
    assert fingertip_rel_pos.shape == (
        N,
        N_FINGERTIPS,
        3,
    ...
    # keypoint positions
    N_KEYPOINTS = 4
    object_keypoint_positions = _compute_keypoint_positions(pose=object_pose, scales=object_scales)
    t7 = time.time()
    ...
    for name, names in OBS_NAME_TO_NAMES.items():
        ...
        assert obs_dict[name].shape[1] == len(
            names
...
def compute_joint_pos_targets(
    ...
    cur_targets[:, :7] = (
        arm_moving_average * cur_targets[:, :7] + (1.0 - arm_moving_average) * prev_targets[:, :7]
    )
    return cur_targets
...
def _compute_palm_center_pos_and_rot(
    ...
    assert T_W_Rs.shape == (1, 4, 4), f"T_W_Rs.shape: {T_W_Rs.shape}, expected: (1, 4, 4)"
    t02 = time.time()
...
def _compute_fingertip_positions_with_offsets(
    ...
    assert T_W_Rs.shape == (1, 1, 4, 4), f"T_W_Rs.shape: {T_W_Rs.shape}, expected: (1, 1, 4, 4)"

    T_W_Fs = T_W_Rs @ T_R_Fs
    fingertip_positions = T_W_Fs[:, :, :3, 3]
    fingertip_rots = T_W_Fs[:, :, :3, :3]
    fingertip_quat_xyzw = matrix_to_quaternion_xyzw_scipy(fingertip_rots.reshape(-1, 3, 3)).reshape(
        N, N_FINGERTIPS, 4
    )

    fingertip_offsets = FINGERTIP_OFFSETS_np[None].repeat(N, axis=0)
    assert fingertip_offsets.shape == (
        N,
        N_FINGERTIPS,
        3,
...
def _compute_keypoint_positions(
    ...
    object_keypoint_offsets = (
        OBJECT_KEYPOINT_OFFSETS_np[None] * OBJECT_BASE_SIZE * KEYPOINT_SCALE / 2 * scales[:, None]
    )
    N_KEYPOINTS = 4
